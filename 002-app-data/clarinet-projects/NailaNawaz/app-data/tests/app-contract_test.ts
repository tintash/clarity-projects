...
Clarinet.test({
    name: "Ensure that valid contract owner can be added and can call get-sum function",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let block = chain.mineBlock([
            Tx.contractCall(
                'data-contract',
                'add-valid-caller',
                [types.principal(`${deployer.address}.app-contract`)],
                deployer.address
...
Clarinet.test({
    name: "Ensure that invalid contract owner cannot call get-sum function",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!
        const trait = `${deployer.address}.data-contract`
