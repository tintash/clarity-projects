...
(begin
   ...
   (let ((doubled-price (* u2 price)))
      ...
      (asserts! (map-insert orders {buyer: buyer-address, seller: tx-sender, item: item-name} {item-received: false, price: price}) (err record-update-error))
      (ok success)
   )
...
(begin
   (let
      ...
      (if (> item-price u0)
       (begin 
          (unwrap! (as-contract (stx-transfer? item-price tx-sender buyer-address)) (err duplicate-record-error))
          (unwrap! (as-contract (stx-transfer? seller-funds tx-sender seller-address)) (err duplicate-record-error))
          (ok success)
       )
       (err no-record-error) 
      )  )
