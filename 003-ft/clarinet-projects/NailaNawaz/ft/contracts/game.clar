...
(define-public (play-game)
    (let
        ...
        (if (> player-balance min-needed-coins)
            ...
            ;; if player does not have enough coins, don't let him play games
            (begin
                (print "not enough coins to play game")
                (ok false)
            )
        )
    )
...
;; transfer coins to some other address
...
(define-private (destroy (amount uint) (address principal))
  (begin
    ...
    (asserts! (or (is-eq tx-sender address)
                  (is-eq tx-sender TOKEN_OWNER))
              (err u100))
    (ft-burn? game-coins amount address))
)
...
(define-read-only (get-balance-of (account principal))
    (ok (ft-get-balance game-coins account) )
)

(define-read-only (get-total-supply)
    (ok (ft-get-supply game-coins))
)
...
;;add some block height to start generating random numbers
